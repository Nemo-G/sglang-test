...
async def async_request_openai_completions(
    ...
    async with aiohttp.ClientSession(timeout=AIOHTTP_TIMEOUT) as session:
        # request_func_input.model = "GLM-4.6-FP8"  # don't hardcode; pass via CLI args
        #request_func_input.model = "Qwen3-Coder-480B-A35B-Instruct-FP8"
        payload = {
        ...
        try:
            async with session.post(
                ...
                if response.status == 200:
                    ...
                    error_text = f"HTTP {response.status}: {response.reason or 'Unknown error'}"
                    try:
                        response_text = await response.text()
                        error_text += f" - Response: {response_text[:500]}"
                    except:
                        pass
...
def get_dataset(args, tokenizer):
    print("get_dataset====> ", args, tokenizer)
...
async def benchmark(
    ...
    # Warmup
    if hasattr(args, 'skip_warmup') and args.skip_warmup:
        print("Skipping warmup test as requested...")
    else:
        ...
        try:
            import requests
            print(f"Testing base URL connectivity...")
            response = requests.get(base_url, timeout=5)
            print(f"Base URL {base_url} responded with status: {response.status_code}")
        except Exception as base_error:
            print(f"Cannot reach base URL {base_url}: {base_error}")
        
        # Test if the API endpoint exists
        try:
            print(f"Testing API endpoint {api_url}...")
            response = requests.post(api_url, json={"test": "connection"}, timeout=5)
            print(f"API URL {api_url} responded with status: {response.status_code}")
        ...
        print(f"Test prompt length: {test_prompt_len}")
        print(f"Test output length: {min(test_output_len, 32)}")
        ...
        if not test_output.success:
            ...
            print(f"Success: {test_output.success}")
            print(f"Latency: {test_output.latency}")
            print("=====================================")
...
def run_benchmark(args_: argparse.Namespace):
    ...
    # args.model = "Qwen/Qwen3-235B-A22B-Instruct-2507"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Model id string used in the OpenAI-compatible request payload. Can be unrelated to --model path.",
    )
    parser.add_argument(
        "--tokenizer",
        type=str,
    ...
    parser.add_argument(
        "--skip-warmup",
        action="store_true",
        help="Skip the initial warmup test (useful for debugging)",
    )
